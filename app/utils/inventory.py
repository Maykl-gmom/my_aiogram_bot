...
# Директорії та налаштування
DATA_DIR = "data"
STOCK_DIR = os.path.join(DATA_DIR, "stock")
RESERVED_DIR = os.path.join(DATA_DIR, "reserved")
PROCESSED_DIR = os.path.join(DATA_DIR, "processed")

DB_PATH = os.path.join(DATA_DIR, "db.sqlite3")

RESERVE_SECONDS = int(get_reserve_seconds())
ALLOWED_EXTS = set(get_allowed_exts())
...
def list_stock_files(category: str, product: str) -> List[str]:
    """Повертає список файлів (за часом зростання mtime) у stock/<cat>/<product>, фільтр за ALLOWED_EXTS."""
...
def _row_to_reservation(r: sqlite3.Row) -> Reservation:
    return Reservation(
        ...
        stock_path=str(r["stock_path"]),
        reserved_path=str(r["reserved_path"]),
        status=str(r["status"]),
...
def fetch_reservation(res_id: int) -> Optional[Reservation]:
    with db() as conn:
        r = conn.execute("SELECT * FROM reservations WHERE id=?", (int(res_id),)).fetchone()
        if not r:
            return None
        return _row_to_reservation(r)
...
def reserve_first_file(category: str, product: str, user_id: int) -> Optional[Reservation]:
    ...
    with db() as conn:
        conn.execute(
            ...
            INSERT INTO reservations(
                user_id, category, product, file_name,
    ...
    return fetch_reservation(int(res_id))


def cancel_reservation(res_id: int) -> bool:
    r = fetch_reservation(int(res_id))
    ...
    with db() as conn:
        conn.execute("UPDATE reservations SET status='canceled' WHERE id=?", (int(res_id),))
    return True
...
def complete_reservation(res_id: int) -> Optional[str]:
    ...
    if os.path.exists(r.reserved_path):
        shutil.move(r.reserved_path, dest_path)
    with db() as conn:
        conn.execute("UPDATE reservations SET status='completed' WHERE id=?", (int(res_id),))
    return dest_path
...
def expire_overdue() -> List[int]:
    ...
    with db() as conn:
        rows = conn.execute(
            "SELECT id FROM reservations WHERE status='reserved' AND expires_at < ?",
            (now,),
        ).fetchall()
    ...
    for cat in sorted(os.listdir(STOCK_DIR)):
        ...
        for prod in sorted(os.listdir(cat_dir)):
            ...
            if not os.path.isdir(prod_dir):
                continue
            cnt = len(
                [f for f in os.listdir(prod_dir) if not f.startswith(".") and _allowed_file(f)]
            )
            result[(cat, prod)] = cnt
    return result
