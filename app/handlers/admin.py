...
from app.utils.inventory import complete_reservation, cancel_reservation
...
async def admin_actions(callback: CallbackQuery):
    ...
    action = parts[1]
    res_id_str, user_id_str = parts[2], parts[3]
    try:
    ...
    if action == "approve":
        ...
        if not path:
            await safe_answer(
                callback, "Не вдалося завершити резерв (можливо, вже завершений).", show_alert=True
            )
            return
        ...
        await callback.message.edit_text(
            f"✅ Підтверджено. Відправив файл користувачу id={user_id}."
        )
        ...
        try:
            await callback.message.bot.send_message(
                chat_id=user_id, text="Оплату не підтверджено. Резерв скасовано."
            )
        except Exception:
            pass
...
async def approve_topup(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"✅ Поповнення {amount} грн підтверджено. Баланс користувача: {new_bal} грн."
    )
    try:
        await callback.message.bot.send_message(
            user_id, f"✅ Адмін підтвердив поповнення на {amount} грн. Ваш баланс: {new_bal} грн."
        )
    except Exception:
        pass
...
async def reject_topup(callback: CallbackQuery):
    ...
    try:
        await callback.message.bot.send_message(
            user_id, "❌ Адмін відхилив поповнення. Звірте дані платежу та спробуйте ще раз."
        )
    except Exception:
        pass
    await safe_answer(callback, "Відхилено")
